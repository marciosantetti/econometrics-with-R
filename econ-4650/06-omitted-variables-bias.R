...
#=============================================================================#
#                OMITTED VARIABLES BIAS/ MISSPECIFICATION ISSUES              #
#=============================================================================#
...
omit_model %>% summary()   ## what happened?


## Comparing the models' coefficients:

true_model %>% tidy() %>% select(term, estimate)
omit_model %>% tidy() %>% select(term, estimate)     ## what happened?
...
mroz_data %>% ggplot(mapping = aes(x=hwage, y=log(faminc))) + 
  geom_point() + theme_minimal() 
  ## is there a relationship between 'hwage' and 'log(faminc)'?


## Let us now inspect the residuals from these two models:


resid_true <- true_model %>% resid()
resid_omit <- omit_model %>% resid()
...
mroz_data %>% summarize(cor_hwage_resid_true = cor(hwage, resid_true),
                        cor_hwage_resid_omit = cor(hwage, resid_omit))


mroz_data <- mroz_data %>% cbind(resid_true, resid_omit)
...
p1 <- mroz_data %>% ggplot(mapping = aes(x=resid_true, y=hwage)) + 
  geom_point() + geom_smooth(method="lm", se=FALSE) + theme_minimal()

p2 <- mroz_data %>% ggplot(mapping = aes(x=resid_omit, y=hwage)) + 
  geom_point() + geom_smooth(method="lm", se=FALSE) + theme_minimal()
...
irrel_model %>% summary()  ## what happened?
...
true_model %>% tidy() %>% select(term, estimate)
irrel_model %>% tidy() %>% select(term, estimate)     ## what happened? 


## Comparing standard errors:

true_model %>% tidy() %>% select(term, std.error)
irrel_model %>% tidy() %>% select(term, std.error)     ## what happened?
...
mroz_data <- mroz_data %>% cbind(y_hat_sq, y_hat_cb)
...
aux_reg_reset %>% summary()


## Are these two added terms jointly significant? Let's run an F-test:


aux_reg_reset %>% linearHypothesis(c("y_hat_sq = 0", "y_hat_cb = 0"))  ## what do we conclude?
...
true_model %>% resettest(power = 2:3)   ## is the F-test statistic the same as above?
...
true_model2 %>% resettest(power = 2:3)
...
true_model3 %>% resettest(power = 2:3)
...
true_model4 %>% resettest(power = 2:3)
